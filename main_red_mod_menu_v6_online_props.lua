...
Menu.Colors = {
    HeaderPink = { r = 255, g = 56, b = 106 },
    SelectedBg = { r = 255, g = 56, b = 106 },
    TextWhite = { r = 247, g = 248, b = 250 },
    TextMuted = { r = 136, g = 141, b = 151 },
    BackgroundDark = { r = 11, g = 12, b = 16 },
    FooterBlack = { r = 9, g = 10, b = 13 },
    Border = { r = 33, g = 36, b = 44 },
    TabInactive = { r = 16, g = 18, b = 23 },
    ItemBackground = { r = 17, g = 19, b = 24 },
    ItemSelectedOverlay = { r = 58, g = 22, b = 38 },
    CardBackground = { r = 18, g = 20, b = 26 },
    HeaderGlow = { r = 255, g = 94, b = 136 }
}
...
local selectedOnlinePlayerIndex = 1
...
function Menu.RefreshOnlinePlayers()
    ...
    for i = 0, 255 do
        if NetworkIsPlayerActive(i) then
            local playerName = GetPlayerName(i) or ("Player " .. tostring(i))
            local serverId = GetPlayerServerId(i)
            table.insert(onlinePlayerOptions, playerName .. " [" .. tostring(serverId) .. "]")
        end
    end

    if #onlinePlayerOptions == 0 then
        table.insert(onlinePlayerOptions, "No players found")
    end

    if selectedOnlinePlayerIndex > #onlinePlayerOptions then
        selectedOnlinePlayerIndex = 1
    end
end
...
function Menu.SpawnSelectedPropNearMe()
    CreateThread(function()
        ...
        local spawnX = coords.x + (forward.x * 2.0)
        local spawnY = coords.y + (forward.y * 2.0)
...
function Menu.PrintSelectedPlayer()
    local selected = onlinePlayerOptions[selectedOnlinePlayerIndex] or "None"
...
if not Menu.TopLevelTabs then
    Menu.TopLevelTabs = { onlineToolsTab }
else
    local exists = false
...
function Menu.DrawHeader()
    ...
    Menu.DrawRect(x + 14, y + 12, width - 28, height - 24, Menu.Colors.CardBackground.r, Menu.Colors.CardBackground.g, Menu.Colors.CardBackground.b, 135)
    Menu.DrawRect(x, y + height - 3, width, 3, Menu.Colors.SelectedBg.r, Menu.Colors.SelectedBg.g, Menu.Colors.SelectedBg.b, 255)
    Menu.DrawRect(x + 22, y + 18, 3, height - 36, Menu.Colors.SelectedBg.r, Menu.Colors.SelectedBg.g, Menu.Colors.SelectedBg.b, 255)

    Menu.DrawText(x + 40, y + 16, "MACHO", 24, 255, 255, 255, 255)
    Menu.DrawText(x + 40, y + 48, "premium dark ui", 11, Menu.Colors.TextMuted.r, Menu.Colors.TextMuted.g, Menu.Colors.TextMuted.b, 220)
end
...
function Menu.DrawTabs(category, x, startY, width, tabHeight)
    ...
    for i, tab in ipairs(category.tabs) do
        ...
        if isSelected then
            ...
            Menu.DrawRect(selectorX, startY + (4 * scale), (4 * scale), tabHeight - (8 * scale), Menu.Colors.SelectedBg.r, Menu.Colors.SelectedBg.g, Menu.Colors.SelectedBg.b, 255)
        end

        local tabBg = isSelected and Menu.Colors.CardBackground or Menu.Colors.TabInactive
        Menu.DrawRect(tabX, startY, currentTabWidth, tabHeight, tabBg.r, tabBg.g, tabBg.b, isSelected and 230 or 220)
...
function Menu.DrawItem(x, itemY, width, itemHeight, item, isSelected)
    ...
    if item.isSeparator then
        Menu.DrawRect(x, itemY, width, itemHeight, Menu.Colors.ItemBackground.r, Menu.Colors.ItemBackground.g, Menu.Colors.ItemBackground.b, 228)
    ...
    if isSelected then
        ...
        Menu.DrawRect(x + (8 * scale), drawY + (4 * scale), width - (16 * scale), itemHeight - (8 * scale), Menu.Colors.ItemSelectedOverlay.r, Menu.Colors.ItemSelectedOverlay.g, Menu.Colors.ItemSelectedOverlay.b, 110)
        ...
        Menu.DrawRect(selectorX + (2 * scale), drawY + (5 * scale), 4, itemHeight - (10 * scale), Menu.Colors.SelectedBg.r, Menu.Colors.SelectedBg.g, Menu.Colors.SelectedBg.b, 255)
    end

    local textX = x + (24 * scale)
